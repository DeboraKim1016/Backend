...
public class ImageToBase64 implements PlayerImageConverter<File, String> {
	//이미지 파일을 byte 문자열 형태로 변환할 때 
	//base64(이미지 기본 인코딩 방식)를 활용하여 인코딩하는 클래스

	@Override
	public String converte(File f) throws IOException {
		
		//1. 파일 가져오기->commons-io 라이브러리
		//commons-io 라이브러리: 파일, 디렉토리 조작, 입출력, 파일 필터링 등 기능 제공
		
		//readFileToByteArray: 파일을 가져와서 byte 배열 형태로 읽는 메소드
		//픽셀을 byte로 변경, 이미지에는 픽셀이 여러 개 있으므로 배열형태로 반환
